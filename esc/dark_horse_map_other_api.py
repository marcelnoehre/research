...
def build_context(n_countries=10, min_championed_by=3):
    ...
    votes_raw = fetch("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2022/2022-05-17/eurovision-votes.csv","/tmp/eurovision-votes.csv")
    esc_raw   = fetch("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2022/2022-05-17/eurovision.csv","/tmp/eurovision.csv")

    finals = esc_raw[esc_raw["section"]=="grand-final"].dropna(subset=["rank","artist_country"]).copy()
    finals["rank"] = finals["rank"].astype(int)
    rank_map = {(r.year,r.artist_country): r["rank"] for _,r in finals.iterrows()}
    for year,d in SUPPLEMENT.items():
        for _,row in pd.read_csv(io.StringIO(d["results"])).iterrows():
            rank_map[(year,row["country"])] = row["rank"]

    votes_f = votes_raw[(votes_raw["semi_final"]=="f")&(votes_raw["duplicate"].isna()|(votes_raw["duplicate"]!="x"))].copy()
    supp=[]
    for year,d in SUPPLEMENT.items():
        v=pd.read_csv(io.StringIO(d["votes"])); v["year"]=year; v["semi_final"]="f"; v["jury_or_televoting"]="C"; v["duplicate"]=None
        supp.append(v[["year","semi_final","from_country","to_country","points","jury_or_televoting","duplicate"]])
    votes_f = pd.concat([votes_f]+supp,ignore_index=True)

    va = votes_f.groupby(["year","from_country","to_country"],as_index=False)["points"].sum()
    ...
    dhc = st[(st["dh_rate"]>=st["dh_rate"].quantile(0.75))&(st["dh_votes"]>=5)].index.tolist()
